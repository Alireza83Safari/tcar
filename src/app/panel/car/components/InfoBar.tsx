...
export async function InfoBar({ cars }: { cars: CarType[] }) {
  ...
  const datas = [
    ...
    { title: "تعداد خودرو نو", dataLength: petrolCarLenth },
    { title: "تعداد خودرو بنزینی", dataLength: cars?.length },
  ];
  return (
    <div className="grid lg:grid-cols-4 sm:grid-cols-2 mt-12">
      {datas?.map((info) => (
        <div className="flex justify-center bg-black-200 rounded-xl max-h-[10rem] mx-4 py-2 lg:mb-0 mb-7">
